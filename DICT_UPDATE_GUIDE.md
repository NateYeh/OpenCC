...
>
> ⚠️ **identity 值要逐字確認**：`岳不群` 三字繁中同形，值可寫 `岳不群`；
> 但 `岳灵珊` 的 `灵` 必須轉 `靈`，值應為 `岳靈珊`——寫成 `岳灵珊` 會把
> 字級 `灵→靈` 一起擋掉（實測：`与岳灵珊 → 與岳灵珊` ❌）。

**驗證**：`岳不群`／`岳灵珊`／`与岳灵珊`／`这是岳灵珊` 皆正確，
且 `五岳`／`东岳`／`山岳`／`岳父`／`岳飞` 輸出零變化；
